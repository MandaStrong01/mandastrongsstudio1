...
// ─────────────────────────────────────────────────────────────────────────────
// HARDCODED WINDOWSILL SCENE RENDERER
// Draws the "man in the window / guitar / moonlit sea / candle" scene entirely
// in canvas — no API call required.
// ─────────────────────────────────────────────────────────────────────────────
function windowsillRenderFilm(ctx, W, H, t, sec, totalSec, beatNow) {
  // ── ACT BOUNDARIES ──────────────────────────────────────────────────────────
  // Act 1  0.00–0.12  opening titles
  // Act 2  0.12–0.35  establish wide shot — room, window, sea
  // Act 3  0.35–0.55  tighten on figure, guitar movement
  // Act 4  0.55–0.75  candle close-up, curtain detail
  // Act 5  0.75–0.90  moon on water, emotion peak
  // Act 6  0.90–1.00  fade to black

  const ease = (a,b,x) => a + (b-a)*Math.max(0,Math.min(1,x));

  // Slow camera drift — far elements move least
  const drift = t * W * 0.03;

  // ── SKY / BACKGROUND ────────────────────────────────────────────────────────
  // Deep night sky gradient
  const sky = ctx.createLinearGradient(0, 0, 0, H * 0.65);
  sky.addColorStop(0,  "rgb(2,4,18)");
  sky.addColorStop(0.5,"rgb(4,8,32)");
  sky.addColorStop(1,  "rgb(6,12,40)");
  ctx.fillStyle = sky;
  ctx.fillRect(0, 0, W, H * 0.65);

  // ── STARS ───────────────────────────────────────────────────────────────────
  const starSeed = [37,71,113,149,193,229,257,293,317,353,389,401,421,443,467,491,509,523,547,557];
  starSeed.forEach((s, i) => {
    const sx = (s * 137 % W);
    const sy = (s * 97  % (H * 0.52));
    const twinkle = 0.4 + 0.6 * Math.abs(Math.sin(sec * 0.7 + i));
    const r = i % 5 === 0 ? 1.5 : 0.8;
    ctx.beginPath();
    ctx.arc(sx - drift * 0.05, sy, r, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(255,255,240,${twinkle * 0.9})`;
    ctx.fill();
  });

  // ── FULL MOON ────────────────────────────────────────────────────────────────
  const moonX = W * 0.64 - drift * 0.08;
  const moonY = H * 0.17;
  const moonR = H * 0.075;
  // Outer glow
  const moonGlow = ctx.createRadialGradient(moonX, moonY, 0, moonX, moonY, moonR * 3.5);
  moonGlow.addColorStop(0,   "rgba(255,255,230,0.18)");
  moonGlow.addColorStop(0.4, "rgba(255,255,210,0.07)");
  moonGlow.addColorStop(1,   "rgba(255,255,200,0)");
  ctx.fillStyle = moonGlow;
  ctx.fillRect(moonX - moonR*4, moonY - moonR*4, moonR*8, moonR*8);
  // Moon disc
  const moonDisc = ctx.createRadialGradient(moonX - moonR*0.2, moonY - moonR*0.15, moonR*0.1, moonX, moonY, moonR);
  moonDisc.addColorStop(0,   "rgba(255,255,245,1)");
  moonDisc.addColorStop(0.6, "rgba(240,240,220,0.95)");
  moonDisc.addColorStop(1,   "rgba(220,220,200,0.85)");
  ctx.beginPath();
  ...
  seaGrad.addColorStop(0,   "rgb(5,12,38)");
  seaGrad.addColorStop(0.4, "rgb(4,10,30)");
  seaGrad.addColorStop(1,   "rgb(2,6,20)");
  ctx.fillStyle = seaGrad;
  ctx.fillRect(0, horizonY, W, H - horizonY);

  // Moonlight shimmer path on water
  const shX = moonX;
  const shimmer = ctx.createLinearGradient(shX - 55, horizonY, shX + 55, H);
  shimmer.addColorStop(0,   "rgba(255,255,210,0)");
  shimmer.addColorStop(0.3, "rgba(255,255,200,0.22)");
  shimmer.addColorStop(0.7, "rgba(255,255,190,0.12)");
  shimmer.addColorStop(1,   "rgba(255,255,180,0)");
  ctx.fillStyle = shimmer;
  ctx.fillRect(shX - 55, horizonY, 110, H - horizonY);

  // Animated wave layers
  for (let w = 0; w < 6; w++) {
    const wAlpha = 0.12 + w * 0.06;
    const wY = horizonY + w * 22;
    const wSpeed = 0.3 + w * 0.12;
    const wAmp = 8 + w * 3;
    ctx.strokeStyle = `rgba(${20+w*10},${50+w*15},${110+w*12},${wAlpha})`;
    ctx.lineWidth = 1.5 + w * 0.4;
    ctx.beginPath();
    for (let x = 0; x <= W; x += 4) {
      const y = wY + Math.sin(x * 0.009 + sec * wSpeed + w) * wAmp
                   + Math.sin(x * 0.022 + sec * (wSpeed * 1.8)) * (wAmp * 0.4);
      x === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
    }
    ctx.stroke();
  }

  // ── ROOM INTERIOR (dark wood walls) ─────────────────────────────────────────
  const roomGrad = ctx.createLinearGradient(0, 0, W, H);
  roomGrad.addColorStop(0,   "rgba(8,5,2,0.97)");
  roomGrad.addColorStop(0.4, "rgba(6,4,1,0.95)");
  roomGrad.addColorStop(1,   "rgba(4,3,1,0.98)");
  ...
  floorGrad.addColorStop(0, "rgba(12,8,3,0.9)");
  floorGrad.addColorStop(1, "rgba(6,4,1,0.98)");
  ...
  const winW = W * 0.44;
  const winH = H * 0.72;
  // Window sill / reveal depth
  ctx.fillStyle = "rgba(30,20,8,0.95)";
  ctx.fillRect(winX - 14, winY - 8, winW + 28, winH + 16);
  // Frame border
  ctx.strokeStyle = "rgba(55,35,12,0.98)";
  ctx.lineWidth = 14;
  ...
  // Vertical cross bar
  ctx.beginPath();
  ctx.moveTo(winX + winW * 0.52, winY);
  ctx.lineTo(winX + winW * 0.52, winY + winH);
  ctx.stroke();

  // ── CURTAINS ────────────────────────────────────────────────────────────────
  const ctTime = sec * 0.55;
  // Left curtain
  ctx.strokeStyle = "rgba(160,135,105,0.45)";
  ctx.lineWidth = 4;
  for (let fold = 0; fold < 3; fold++) {
    const cx0 = winX - 6 + fold * 8;
    ctx.beginPath();
    ctx.moveTo(cx0, winY - 2);
    ctx.bezierCurveTo(
      cx0 + Math.sin(ctTime + fold) * 14, winY + winH * 0.35,
      cx0 + Math.sin(ctTime + fold + 1) * 18, winY + winH * 0.65,
      cx0 + Math.sin(ctTime + fold + 2) * 10, winY + winH
    );
    ctx.stroke();
  }
  // Right curtain — slight breeze drift
  const rcX = winX + winW + 6;
  for (let fold = 0; fold < 3; fold++) {
    const cx0 = rcX - fold * 8;
    ctx.beginPath();
    ctx.moveTo(cx0, winY - 2);
    ctx.bezierCurveTo(
      cx0 - Math.sin(ctTime * 0.8 + fold + 0.5) * 16, winY + winH * 0.3,
      cx0 - Math.sin(ctTime * 0.8 + fold + 1.5) * 22, winY + winH * 0.6,
      cx0 - Math.sin(ctTime * 0.8 + fold + 2.5) * 12, winY + winH
    );
    ctx.stroke();
  }

  // ── SEATED FIGURE ON WINDOWSILL ─────────────────────────────────────────────
  // Positioned at right side of window frame, torso slightly twisted toward sea
  const fx = winX + winW * 0.58;
  const fy = winY + winH * 0.52;
  const figScale = H * 0.0028;

  ctx.fillStyle = "rgba(0,0,0,0.96)";
  ctx.strokeStyle = "rgba(0,0,0,0.96)";

  // Head (facing slightly left — toward sea)
  ctx.beginPath();
  ctx.ellipse(fx - 2, fy - 78 * figScale, 16 * figScale, 18 * figScale, -0.12, 0, Math.PI * 2);
  ctx.fill();

  // Neck
  ctx.fillRect(fx - 4 * figScale, fy - 62 * figScale, 8 * figScale, 14 * figScale);

  // Torso (slight twist — shoulders angled)
  ctx.beginPath();
  ctx.moveTo(fx - 22 * figScale, fy - 48 * figScale);  // left shoulder
  ctx.lineTo(fx + 18 * figScale, fy - 52 * figScale);  // right shoulder (back)
  ctx.lineTo(fx + 16 * figScale, fy + 8 * figScale);   // right hip
  ctx.lineTo(fx - 20 * figScale, fy + 6 * figScale);   // left hip
  ctx.closePath();
  ctx.fill();

  // Left arm reaching to guitar neck
  ctx.lineWidth = 11 * figScale;
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(fx - 18 * figScale, fy - 38 * figScale);
  ctx.quadraticCurveTo(fx - 30 * figScale, fy - 10 * figScale, fx - 14 * figScale, fy + 22 * figScale);
  ctx.stroke();

  // Right arm strumming/picking
  const strumOsc = Math.sin(sec * 2.8) * 4 * figScale;
  ctx.beginPath();
  ctx.moveTo(fx + 14 * figScale, fy - 38 * figScale);
  ctx.quadraticCurveTo(fx + 36 * figScale, fy - 8 * figScale, fx + 28 * figScale + strumOsc, fy + 18 * figScale);
  ctx.stroke();

  // Guitar body
  ctx.fillStyle = "rgba(0,0,0,0.94)";
  ctx.beginPath();
  ctx.ellipse(fx + 18 * figScale, fy + 14 * figScale, 22 * figScale, 28 * figScale, 0.25, 0, Math.PI * 2);
  ctx.fill();
  // Guitar waist indent
  ctx.fillStyle = "rgba(4,3,1,0.8)";
  ctx.fillRect(fx + 4 * figScale, fy + 4 * figScale, 14 * figScale, 8 * figScale);
  // Guitar neck
  ctx.fillStyle = "rgba(0,0,0,0.96)";
  ctx.save();
  ctx.translate(fx - 4 * figScale, fy - 12 * figScale);
  ctx.rotate(-0.22);
  ctx.fillRect(-3 * figScale, -52 * figScale, 6 * figScale, 56 * figScale);
  ctx.restore();

  // Legs dangling from sill
  ctx.strokeStyle = "rgba(0,0,0,0.96)";
  ctx.lineWidth = 14 * figScale;
  ctx.lineCap = "round";
  // Left leg
  ctx.beginPath();
  ctx.moveTo(fx - 12 * figScale, fy + 10 * figScale);
  ctx.quadraticCurveTo(fx - 16 * figScale, fy + 55 * figScale, fx - 10 * figScale, fy + 90 * figScale);
  ctx.stroke();
  // Right leg
  ctx.beginPath();
  ctx.moveTo(fx + 8 * figScale, fy + 10 * figScale);
  ctx.quadraticCurveTo(fx + 14 * figScale, fy + 55 * figScale, fx + 10 * figScale, fy + 90 * figScale);
  ctx.stroke();

  // Windowsill ledge the figure sits on
  ctx.fillStyle = "rgba(28,18,6,0.98)";
  ctx.fillRect(winX - 14, winY + winH * 0.44, winW + 28, 16);

  // ── CANDLE (right side, on small table) ─────────────────────────────────────
  const cx2 = W * 0.73 - drift * 0.4;
  const cy2 = H * 0.60;
  const flicker = 0.88 + Math.sin(sec * 9.1) * 0.07 + Math.sin(sec * 14.3) * 0.04 + Math.random() * 0.02;

  // Table surface
  ctx.fillStyle = "rgba(22,14,4,0.95)";
  ctx.fillRect(cx2 - 28, cy2 + 38, 56, 8);
  ctx.fillRect(cx2 - 22, cy2 + 46, 8, 60);
  ctx.fillRect(cx2 + 14, cy2 + 46, 8, 60);

  // Candle body
  ctx.fillStyle = "rgba(235,215,175,0.92)";
  ctx.fillRect(cx2 - 7, cy2, 14, 40);

  // Room glow from candle
  const roomGlow = ctx.createRadialGradient(cx2, cy2, 0, cx2, cy2, W * 0.38);
  roomGlow.addColorStop(0, "rgba(255,145,30,0.14)");
  roomGlow.addColorStop(0.5,"rgba(255,120,20,0.04)");
  roomGlow.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = roomGlow;
  ctx.fillRect(0, 0, W, H);

  // Flame
  const flameH = 28 * flicker;
  const fg = ctx.createRadialGradient(cx2, cy2, 0, cx2, cy2 - flameH * 0.4, flameH * 1.6);
  fg.addColorStop(0,   "rgba(255,255,200,0.98)");
  fg.addColorStop(0.25,"rgba(255,200,50,0.85)");
  fg.addColorStop(0.6, "rgba(255,110,10,0.5)");
  fg.addColorStop(1,   "rgba(255,60,0,0)");
  ctx.fillStyle = fg;
  ctx.beginPath();
  ctx.ellipse(cx2 + Math.sin(sec * 6) * 1.5, cy2 - flameH * 0.5, flameH * 0.38, flameH * 0.9, 0, 0, Math.PI * 2);
  ctx.fill();
  ...
  if (beatNow) {
    ...
    ctx.translate(-W / 2, -H / 2);
    // brief bright flash on candle
    ctx.fillStyle = "rgba(255,180,40,0.06)";
    ctx.fillRect(0, 0, W, H);
    ctx.restore();
  }

  // ── ACT-BASED OVERLAYS ───────────────────────────────────────────────────────
  // Act 3 onward: slight push-in zoom toward figure
  if (t > 0.35 && t < 0.75) {
    const zoomProgress = (t - 0.35) / 0.4;
    ...
    ctx.scale(zoom, zoom);
    ctx.translate(-ox * 0.5, -oy * 0.5);
    // re-draw vignette inside zoom — handled below, just restore
    ctx.restore();
  }

  // Act 5: moon / sea reflection intensifies
  if (t > 0.75 && t < 0.90) {
    const a = (t - 0.75) / 0.15;
    const extraShimmer = ctx.createLinearGradient(moonX - 70, horizonY, moonX + 70, H);
    extraShimmer.addColorStop(0, "rgba(255,255,210,0)");
    extraShimmer.addColorStop(0.5, `rgba(255,255,200,${a * 0.18})`);
    extraShimmer.addColorStop(1, "rgba(255,255,180,0)");
    ctx.fillStyle = extraShimmer;
    ctx.fillRect(moonX - 70, horizonY, 140, H - horizonY);
  }

  // ── COLOUR GRADE ─────────────────────────────────────────────────────────────
  // Cool blue night tint
  ctx.fillStyle = "rgba(0,15,40,0.10)";
  ctx.fillRect(0, 0, W, H);
  // Warm low (candle influence in lower third)
  ctx.fillStyle = "rgba(28,8,0,0.07)";
...
function MusicVideoStudio({ onClose, onSave }) {
  ...
  const setProgress = (v) => setRenderProgress(v);
  ...
  const generateVideo = async () => {
    ...
    try {
      addLog("MandaStrong Cinema Engine — loading scene...");
      setRenderProgress(6);
      ...
      // ── USE HARDCODED WINDOWSILL RENDERER ─────────────────────────────────
      addLog("Scene: Man In The Window — hardcoded cinematic renderer");
      addLog("Film renderer ready — "+Math.round(totalDur)+"s at 12fps");
      setRenderProgress(28);

      const renderFn = windowsillRenderFilm;
      ...
      await new Promise(resolve=>{
        ...
        const tick=()=>{
          ...
          setProgress(28+Math.round((frame/totalFrames)*66));
  ...
  const startRender=async()=>{
    ...
    try{
      ...
      for(let ci=0;ci<clips.length;ci++){
        ...
        if(clip.file instanceof File){
          videoPlayed=await new Promise(resolve=>{
            ...
            vid.onloadeddata=async()=>{
              ...
              vid.currentTime=0;try{await vid.play();}catch(e){}
              const startTime=Date.now();
              const draw=()=>{
                if(done2)return;
                const elapsed=(Date.now()-startTime)/1000;
                if(vid.ended||elapsed>=clipDur){vid.pause();finish(true);return;}
        ...
        if(!videoPlayed){
          log("  Video unavailable — regenerating scene from name...");
