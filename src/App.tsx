...
function speakText(voiceId, txt, onStart, onEnd) {
  ...
  const doSpeak = () => {
    ...
    // Auto-select best matching system voice per character
    if (!picked) {
      ...
      if (voiceId==="aurora") {
        // Warm British female — prefer Kate, Serena, Emily, any en-GB female
        ...
        // Deep American male — prefer Daniel, David, any en-US male
        picked = allVoices.find(x=>/daniel|david|alex/i.test(x.name)&&x.lang.startsWith("en-US"))
              || allVoices.find(x=>x.lang==="en-US"&&malePat.test(x.name))
              || allVoices.find(x=>x.lang.startsWith("en")&&malePat.test(x.name));
      } else if (voiceId==="sophia") {
        // Bright Australian female — prefer Karen, any en-AU
        picked = allVoices.find(x=>/karen/i.test(x.name))
              || allVoices.find(x=>x.lang==="en-AU")
              || allVoices.find(x=>x.lang.startsWith("en")&&femalePat.test(x.name));
      } else if (voiceId==="james") {
        // Dry British male — prefer Daniel (en-GB), Oliver, Arthur
        ...
        // Neutral clean female — prefer Samantha, Victoria, Zira
        ...
        // Warm American South male — prefer any en-US male with warmth
