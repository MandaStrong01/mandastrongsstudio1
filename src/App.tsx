          ...
          <div style={{flex:1,padding:20,overflowY:"auto"}}>
            ...
            {activeTab==="settings"&&(
              <div>
                ...
                {(()=>{const MOOD_LIST=["Calm","Tender","Hopeful","Neutral","Serious","Melancholic","Tense","Dramatic","Excited","Happy","Surprised","Fearful","Angry","Sad"];const mIdx=MOOD_LIST.indexOf(mood);const MOOD_COLORS={"Calm":"#60a5fa","Tender":"#f9a8d4","Hopeful":"#34d399","Neutral":"#e8c96d","Serious":"#d4c9a8","Melancholic":"#818cf8","Tense":"#fb923c","Dramatic":"#e8c96d","Excited":"#facc15","Happy":"#4ade80","Surprised":"#38bdf8","Fearful":"#a78bfa","Angry":"#f87171","Sad":"#94a3b8"};const col=MOOD_COLORS[mood]||GOLD;return(<div style={{...Card(),marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{color:GOLD,fontSize:11,fontWeight:900,letterSpacing:2}}>MOOD</span><span style={{background:col+"22",border:`1px solid ${col}`,color:col,padding:"3px 12px",fontSize:12,fontWeight:900,letterSpacing:2}}>{mood.toUpperCase()}</span></div><input type="range" min={0} max={MOOD_LIST.length-1} step={1} value={mIdx<0?3:mIdx} onChange={e=>setMood(MOOD_LIST[+e.target.value])} style={{width:"100%",accentColor:col,marginBottom:8,cursor:"pointer"}}/><div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:GOLDDIM,fontSize:9,letterSpacing:1}}>CALM</span><span style={{color:GOLDDIM,fontSize:9,letterSpacing:1}}>NEUTRAL</span><span style={{color:GOLDDIM,fontSize:9,letterSpacing:1}}>ANGRY/SAD</span></div><div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:8}}>{MOOD_LIST.map((m,i)=><button key={m} onClick={()=>setMood(m)} style={{background:mood===m?col+"33":"#111",border:`1px solid ${mood===m?col:GOLDDIM+"55"}`,color:mood===m?col:GOLDDIM,padding:"2px 7px",cursor:"pointer",fontSize:9,fontWeight:900,fontFamily:"'Rajdhani',sans-serif",letterSpacing:1}}>{m.toUpperCase()}</button>)}</div></div>);})()}
              </div>
            )}
          </div>
        </div>
