    ...
    <div className="min-h-screen bg-gradient-to-br from-[#2d1554]/20 via-black to-[#2d1554]/20 text-white flex flex-col">
      <div className="flex-1 flex flex-col px-4 py-12">
        <div className="max-w-6xl w-full mx-auto">
          <h1 className="text-5xl font-black text-[#7c3aed] mb-4 text-center">Agent Grok 24/7 Help Desk</h1>
          <p className="text-xl text-white/70 text-center mb-8">Online Now - Ready to Assist You</p>

          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="bg-black/30 backdrop-blur-sm rounded-2xl border border-[#2d1554]/30 p-6 text-center">
              <Clock className="w-12 h-12 mx-auto mb-4 text-[#7c3aed]" />
              <h3 className="text-xl font-bold mb-2">24/7 Availability</h3>
              <p className="text-white/70">Get help anytime, day or night</p>
            </div>
            <div className="bg-black/30 backdrop-blur-sm rounded-2xl border border-[#2d1554]/30 p-6 text-center">
              <MessageCircle className="w-12 h-12 mx-auto mb-4 text-[#7c3aed]" />
              <h3 className="text-xl font-bold mb-2">Instant Responses</h3>
              <p className="text-white/70">Quick answers to your questions</p>
            </div>
            <div className="bg-black/30 backdrop-blur-sm rounded-2xl border border-[#2d1554]/30 p-6 text-center">
              <Headphones className="w-12 h-12 mx-auto mb-4 text-[#7c3aed]" />
          ...
          <div className="bg-black/30 backdrop-blur-sm rounded-2xl border border-[#2d1554]/30 p-8 mb-8">
            <h2 className="text-2xl font-bold mb-6 text-[#7c3aed]">Live Chat</h2>

            <div className="bg-black/50 rounded-lg border border-[#2d1554]/30 p-6 mb-4 h-96 overflow-y-auto">
              <div className="space-y-4">
                {messages.length === 0 ? (
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#2d1554] flex items-center justify-center flex-shrink-0">
                      <MessageCircle className="w-5 h-5" />
                    </div>
                    <div className="bg-[#2d1554]/30 rounded-lg p-4 flex-1">
                  ...
                  messages.map((msg) => (
                    <div key={msg.id} className={`flex items-start gap-3 ${msg.is_user ? 'flex-row-reverse' : ''}`}>
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                        msg.is_user ? 'bg-[#2d1554]' : 'bg-[#2d1554]'
                      }`}>
                        <MessageCircle className="w-5 h-5" />
                      </div>
                      <div className={`rounded-lg p-4 flex-1 ${
                        msg.is_user ? 'bg-[#2d1554]/30' : 'bg-[#2d1554]/30'
                      }`}>
            ...
            <div className="flex gap-2">
              <input
                ...
                disabled={sending}
                className="flex-1 px-4 py-3 bg-black border border-[#2d1554]/50 rounded-lg text-white placeholder-white/60 focus:outline-none focus:border-[#7c3aed] disabled:opacity-50"
              />
              <button
                onClick={handleSendMessage}
                disabled={sending || !inputMessage.trim()}
                className="bg-[#2d1554] hover:bg-[#2d1554] disabled:bg-[#2d1554] disabled:cursor-not-allowed text-white font-bold px-8 py-3 rounded-lg transition-all flex items-center gap-2"
              >
