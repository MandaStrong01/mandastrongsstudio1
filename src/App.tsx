...
function renderFilm(ctx, W, H, t, sec, totalSec, beatNow) {`;
      ...
      const body = code.replace(/^function renderFilm\s*\([^)]*\)\s*\{/,"").replace(/\}$/,"");
      ...
      try{
        ...
        const simple = await fetch("https://yrshnpjvplkwbpmmzwvb.supabase.co/functions/v1/claude-proxy",{
          method:"POST",
          headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01"},
      ...
      await new Promise(resolve=>{
        ...
        const tick=()=>{
          ...
          setProgress(30+Math.round((frame/totalFrames)*64));
      ...
      <div style={{maxWidth:820,margin:"0 auto",textAlign:"center"}}>
        ...
        {guideOpen&&(
          <div style={{...Card(),textAlign:"left",marginBottom:14,borderTop:"none"}}>
            ...
            {[["GETTING STARTED","Open the app. Use the ☰ Quick Access menu top left to jump to any of the 23 pages instantly, or use BACK / NEXT buttons in the footer bar."],["PAGE 4 — LOGIN & PRICING","Sign in, create a free account, or browse as guest. Creator $20/mo · Pro $30/mo · Studio $50/mo with 7-day free trial."],["PAGES 5–10 — AI TOOLS","Over 600 professional AI tools across Writing, Voice, Image, Video, Motion and Enhancement. Click any tool card, describe what you want, hit AI CREATE."],["PAGE 6 — VOICE ENGINE","55 cinematic voice characters. Select your voice, paste your script, set Speed, Pitch and Pause sliders, then hit PREPARE & SPEAK. James is your primary documentary narrator."],["PAGE 8 — VIDEO GENERATOR","The heart of your studio. Describe your scene in plain English. Hit GENERATE VIDEO CLIP. The canvas engine builds your cinematic environment. Duration slider is in the left panel controls."],["PAGE 11 — UPLOAD MEDIA","Upload your own video, audio and image files. Drag and drop or click to browse. Everything lands in your Media Library."],["PAGE 13 — TIMELINE EDITOR","Drag your media clips onto the video, audio and title tracks. Build your film sequence. Hit RENDER when ready."],["PAGE 15 — AUDIO MIXER","Set your mix levels. Recommended documentary settings: VOICE 85 · MUSIC 40 · EFX 50 · MASTER 85."],["PAGE 16 — RENDER ENGINE","Choose your output quality up to 4K. Select frame rate and codec. Hit START RENDER. Your film renders in your browser."],["PAGE 17 — FILM PREVIEW","Watch your completed film. Full playback controls."],["PAGE 18 — EXPORT & DISTRIBUTE","Download to device. Share to YouTube, Instagram, TikTok, Facebook, LinkedIn, Vimeo and WhatsApp."],["PAGE 21 — AGENT GROK","Your 24/7 AI studio assistant. Ask anything about production, tools, pricing or workflow."],["MUSIC VIDEO STUDIO","Access from Page 6. Four-step wizard: Music → Vocals → Video → Generate. Enter your scene description on Step 4."],["PRODUCTION WORKFLOW","Recommended order: Page 8 → Page 6 → Page 13 → Page 15 → Page 16 → Page 17 → Page 18."]].map(([t,d])=>(
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    style.textContent=`*{box-sizing:border-box!important;}body,html{margin:0;padding:0;width:100%;overflow-x:hidden;}[data-bolt-badge],a[href*='bolt.new'],.bolt-badge{display:none!important;}@media(max-width:900px){.grid-cols-2,.grid-cols-3,.grid-cols-4{grid-template-columns:1fr 1fr!important;}}@media(max-width:600px){.grid-cols-2,.grid-cols-3,.grid-cols-4{grid-template-columns:1fr!important;}}`;
