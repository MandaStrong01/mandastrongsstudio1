...
export default function App() {
    ...
    <div className="fixed top-6 right-6 z-50 group">
      ...
      <div className="hidden group-hover:block absolute right-0 mt-2 w-56 bg-gray-900 border-2 border-purple-600 rounded-xl shadow-2xl p-2 uppercase italic text-xs font-bold">
        {['Editor Home', 'Media Library', 'Tool Boards', 'Help Desk', 'Community Hub'].map(item => (
          <button key={item} className="w-full text-left p-3 hover:bg-purple-600 rounded-lg border-b border-purple-900/50 last:border-0">{item}</button>
        ))}
      </div>
    </div>
  );
  ...
  const Nav = ({ p, n }: { p: number; n: number }) => (
    <div className="fixed bottom-16 left-1/2 -translate-x-1/2 flex gap-8 z-40">
      <button onClick={() => setCurrentPage(p)} className="bg-purple-950/80 text-white px-14 py-3 rounded-xl font-bold border-2 border-purple-500 uppercase italic">Back</button>
      <button onClick={() => setCurrentPage(n)} className="bg-purple-600 text-white px-14 py-3 rounded-xl font-bold border-2 border-purple-400 uppercase italic">Next</button>
    </div>
  );
  ...
  const renderPage = () => {
    switch (currentPage) {
          ...
          <div className="min-h-screen bg-black/90 text-white p-8 overflow-y-auto pb-48">
            <div className="max-w-7xl mx-auto pt-10">
              <div className="grid md:grid-cols-2 gap-8 mb-20">
                ...
                <div className="bg-purple-950/20 border-2 border-purple-600 rounded-3xl p-10 text-center flex flex-col justify-center"><h2 className="text-3xl font-black mb-8 italic uppercase text-purple-500 underline decoration-purple-600">Register</h2><button className="w-full bg-purple-600 py-4 rounded-xl font-black text-xl uppercase italic">Create Account</button></div>
              </div>
      ...
      case 4: // AI BOARD 1 (SEARCH TOP LEFT)
        return (
        ...
        return (
          <div className="min-h-screen bg-black text-white p-10 flex flex-col pb-48">
            ...
            <div className="flex-1 grid md:grid-cols-2 gap-12 mb-10">
              <div className="bg-gray-900 border-2 border-purple-600 rounded-3xl flex items-center justify-center relative shadow-inner"><Play size={100} className="text-purple-600 opacity-20" /><div className="absolute top-6 left-6 bg-purple-600 px-5 py-2 rounded-xl text-xs font-black uppercase italic shadow-lg">Live Viewer</div></div>
              <div className="bg-gray-950 p-12 rounded-3xl border border-purple-900/30 flex flex-col justify-center">
                ...
                <div className="flex justify-between mt-12 text-6xl font-black text-white tracking-tighter italic"><span>{movieDuration} MIN</span><span className="text-gray-700 text-xl uppercase tracking-[0.3em] self-end">180 Min Max</span></div>
              </div>
            </div>
  ...
  return <div className="min-h-screen bg-black font-sans selection:bg-purple-500 overflow-x-hidden">{renderPage()}<QuickAccess /><GrokBubble /></div>;
}